	...
	then
		...
		then
			...
			echo "$DATESTRING : SUCCESS  : $USER : $TRAN_TYPE PUT ${FILE} ${DEST_USER}@${DEST_HOST}:${DEST_DIR}" >> $MY_LOG
                        touch /home/coretx/outgoing/artemis/artemis.done
                        SFTPBATCH1=".sftp.batch.${DATESTRING}"
                        echo "put /home/coretx/outgoing/artemis/artemis.done $DEST_DIR" > $SFTPBATCH1
                        sftp -b $SFTPBATCH1 ${DEST_USER}@${DEST_HOST}
                        if [ "$RC" -eq 0 ];
                        then
                             echo "$DATESTRING : SUCCESS  : $TRAN_TYPE PUT artemis.done ${DEST_USER}@${DEST_HOST}:${DEST_DIR}"
                             echo "$DATESTRING : SUCCESS  : $USER : $TRAN_TYPE PUT artemis.done ${DEST_USER}@${DEST_HOST}:${DEST_DIR}" >> $MY_LOG
                        else
                             echo "$DATESTRING : FAILED   : $TRAN_TYPE PUT artemis.done ${DEST_USER}@${DEST_HOST}:${DEST_DIR}"
                             echo "$DATESTRING : FAILED   : $USER : $TRAN_TYPE PUT artemis.done ${DEST_USER}@${DEST_HOST}:${DEST_DIR}" >> $MY_LOG
                        fi 
			...
			then
				...
				mv -v ${FILE} ${ARC_DIR}/${FILENAME}.${DATESTRING}
                                rm -vf /home/coretx/outgoing/artemis/artemis.done
			fi
		else
