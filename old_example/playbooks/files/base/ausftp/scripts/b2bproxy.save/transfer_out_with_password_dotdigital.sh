...
for FILENAME in `ls ${SRC_DIR}`; do
        ...
        if [ -f "$FILE" ];
        then
                echo "$DATESTRING : FILE NO  : $COUNT"
                echo "$DATESTRING : TRANSFER : ${FILENAME} to ${DEST_USER}@${DEST_HOST}:${DEST_DIR}/"

                # Batch file constructor
                SFTPBATCH=".sftp.batch.${DEST_USER}.${DATESTRING}"
                echo "put ${FILE} ${DEST_DIR}/" > $SFTPBATCH
                echo "ls ${DEST_DIR}/${FILENAME}" >> $SFTPBATCH
                echo "exit" >> $SFTPBATCH
                ...
                if [ "$RC" -eq 0 ];
                then
                        echo "$DATESTRING : SUCCESS  : PUT ${FILE} ${DEST_USER}@${DEST_HOST}:${DEST_DIR}/"
                        echo "$DATESTRING : SUCCESS  : $USER : PUT ${FILE} ${DEST_USER}@${DEST_HOST}:${DEST_DIR}/" >> $MY_LOG
                        ...
                        echo "$DATESTRING : FAILED   : PUT ${FILE} ${DEST_USER}@${DEST_HOST}:${DEST_DIR}/"
                        echo "$DATESTRING : FAILED   : $USER : PUT ${FILE} ${DEST_USER}@${DEST_HOST}:${DEST_DIR}/" >> $MY_LOG
                fi
        fi
done
echo "$DATESTRING : END"
